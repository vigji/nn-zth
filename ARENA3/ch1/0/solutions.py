...
if MAIN:
    assert reference_gpt2.tokenizer is not None
...
if MAIN:
    ...
    train_loader = DataLoader(
        dataset_dict["train"],  # type: ignore
...
class TransformerSampler:
    ...
    def sample(self, prompt: str, max_tokens_generated=100, verbose=False, **kwargs):
        ...
        """
        self.model.eval()
        input_ids = self.tokenizer.encode(prompt, return_tensors="pt").to(device)[0]  # type: ignore
    ...
    def sample_top_p(
        ...
        return keep_idx[sample].item()  # type: ignore
...
if MAIN:
    ...
    for _ in tqdm(range(N)):
        ...
        frequency_of_top_5[tokenizer.decode(token)] += 1  # type: ignore
...
if MAIN:
    ...
    beams_with_eos = Beams(model, tokenizer, logprob_sums, tokens)  # type: ignore
...
if MAIN:
    ...
    orig_len = len(tokenizer.encode(prompt))  # type: ignore
